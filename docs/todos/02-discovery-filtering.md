...
- [x] 2026-07-04 add Bot3-only FIFA/Sports exact-score auto-buy filtering behind `BOT3_FIFA_EXACT_SCORE_AUTO_BUY_ENABLED`: planned buys remain first priority; non-planned exact-score markets must parse as `A vs B`, exclude Total Goals/Total Score/Goal Differential/Score Different side markets, expose the six canonical win-side scorelines, and pass uniform price-tier checks before Bot3 can auto-select the lower price win-side tier.
