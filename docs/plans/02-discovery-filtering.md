...
- `价格`: only BTC Price/8 hour/clock-curve/point-in-time BTC price events; non-BTC Meme price-range events such as `$PUMP price range...` and FDV markets whose resolution text mentions price, such as `$ARROW FDV...`, remain visible/notifiable
...
- With `MEME_RANGE_SELECTION_ENABLED=1`, Bot2/Bot5 Meme FDV/market-cap/price-range events use one shared first-observation lock. Shared discovery hydrates the event once, follows the event's exact DEX Screener pair or Pyth feed, validates token identity, maps the current value into the numeric outcome ranges, and locks the matched bucket plus its immediate lower and upper neighbors. Edge buckets use the nearest three-bucket window.
- The Meme lock is persisted in `MEME_RANGE_SELECTION_FILE` and embedded in the shared discovery feed. It is never refreshed before opening and is reused after watcher restarts. `MEME_RANGE_SELECTION_OUTCOME_COUNT` controls only profile-local execution around that immutable matched bucket. Bot2 uses five outcomes (two lower, matched, two upper) and falls back to the middle five; edge windows shift to keep five when available. Bot5 retains three outcomes. Profile-level `first`/`middle` remain applicable to non-Meme Binance-strong events.
- Bot2/Bot5 read the same lock file before startup REST preparation, so REST seed cannot outrun the shared feed and freeze a different fallback plan. If a locked feed row arrives after an un-signed fallback plan, the pending plan may be rebuilt once from that original lock; a signed or timer-owned plan is never replaced. Locked Meme price-range events may bypass the generic buy-side Price block, while non-Meme price markets remain blocked.
...
- Profile-local planned buys use `EVENT_PLANNED_BUYS_FILE` to bind a market address or exact question to its own outcome names and per-outcome stake. A planned buy overrides the global `middle` selection only for that market, is marked in the prepared plan, can pass the single-market-per-open limit when the operator explicitly planned multiple same-start markets, and can override the buy-side Price strategy filter because the operator has named an exact market and outcomes.
...
FIFA/Sports exact-score and player-performance prop markets are display/notification classes by default. Bot2/Bot5 do not default-buy them from display alone. The exact-score exception is enabled only for native Bot3 or a profile assigned `EVENT_PROFILE_ROLE=bot3_like`, and still requires `BOT3_FIFA_EXACT_SCORE_AUTO_BUY_ENABLED=1`. Production Bot1 uses that role. With no matching planned buy, these profiles may auto-buy only Sports/FIFA exact-score markets that parse as `A vs B`, are not Total Goals/Total Score/Goal Differential/Score Different side markets, and expose all ten canonical win-side outcomes (`1-0`, `2-0`, `3-0`, `2-1`, `3-1`, `0-1`, `0-2`, `0-3`, `1-2`, `1-3`). The side decision uses the three-score price-tier rows (`1-0`, `2-0`, `2-1` versus `0-1`, `0-2`, `1-2`); the lower-price non-draw side contributes its five-score basket. Draw outcomes remain display-only. Any parse, price, tier, or side-market failure skips auto-buy while preserving operator visibility.

Bot1/Bot3 planned buys remain higher priority than this automatic exact-score selector. A market address/question matched by that profile's `EVENT_PLANNED_BUYS_FILE` must use the operator-specified outcome names, stake, timing, gas, Builder, and auto-sell override, even if the automatic selector would choose the other side.
